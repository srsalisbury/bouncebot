...
func TestRepository_Create(t *testing.T) {
	...
	room, playerID, sessionToken := repo.Create("Alice", false)
	...
	if playerID != room.Players[0].ID {
		t.Error("expected returned playerID to match first player")
	}
	if sessionToken == "" {
		t.Error("expected session token to be set")
	}
	if room.Players[0].SessionToken != sessionToken {
		t.Error("expected player session token to match returned token")
	}
...
func TestRepository_Create_UniqueIDs(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		room, _, _ := repo.Create("Player", false)
		if ids[room.ID] {
...
func TestRepository_Get(t *testing.T) {
	...
	created, _, _ := repo.Create("Alice", false)
...
func TestRepository_Delete(t *testing.T) {
	...
	room, _, _ := repo.Create("Alice", false)
	roomID := room.ID
...
func TestRepository_All(t *testing.T) {
	...
	_, _, _ = repo.Create("Alice", false)
	_, _, _ = repo.Create("Bob", false)
	_, _, _ = repo.Create("Charlie", false)
